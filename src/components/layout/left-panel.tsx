...
export default function LeftPanel({ isCollapsed, onToggleCollapse, loadingChatId: propLoadingChatId, setLoadingChatId: propSetLoadingChatId }: LeftPanelProps) {
  ...
  if (isCollapsed) {
    return (
      <div className="w-16 bg-white h-full flex flex-col items-center py-4 space-y-6 border-r border-gray-200 transition-all duration-500 ease-in-out">
        {/* Logo */}
        <div className="flex items-center space-x-3">
            {/* Logo with normal state LexAmplify and chevron, but on hover (not collapsed) chevron replaces L */}
            <div className="relative group w-8 h-8">
              <button
                onClick={onToggleCollapse}
                className="absolute inset-0 w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10"
                tabIndex={-1}
                aria-label="Collapse sidebar"
              >
                <Icon name="chevronRight" className="w-4 h-4 text-black" />
              </button>
              <span className="text-black font-semibold text-sm w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center group-hover:opacity-0 transition-opacity">
                L
              </span>
            </div>
        ...
        <button
          ...
          className="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center hover:bg-gray-200 transition-all duration-300 ease-in-out hover:scale-105"
        >
          <Icon name="plus" className="w-4 h-4 text-black" />
        </button>

        {/* Search */}
        <button className="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center hover:bg-gray-200 transition-all duration-300 ease-in-out hover:scale-105">
          <Icon name="search" className="w-4 h-4 text-black" />
        </button>

        {/* Storage */}
        <button className="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center hover:bg-gray-200 transition-all duration-300 ease-in-out hover:scale-105">
          <Icon name="folder" className="w-4 h-4 text-black" />
        </button>
